let AKBG = [
    {
        id              : ['sasshii', 'sashi', 'sasshii', 'sashii', 'rino', 'sashihara'],
        name            : {
            kanji       : '指原莉乃',
            hiragana    : 'さしはら りの',
            romaji      : 'Sashihara Rino'
        },
        surname         : {
            kanji       : '指原',
            hiragana    : 'さしはら'
        },
        given_name    : {
            kanji       : '莉乃',
            hiragana    : 'りの'
        },
            romaji        : 'Sashihara Rino',
            nickname      : 'Sasshii (さっしー)',
            date_of_birth : 'November 21, 1992  ',
            blood_type    : 'O',
            from          : ':flag_jp: Oita City, Oita, Japan',
            office        : 'Ohta Pro',
            current_teams : [ 'Former of HKT48 Team H' ],
            catch_phrase  : `大分県出身、さっしーこと指原莉乃です \nOita-ken shusshin, Sasshii koto Sashihara Rino desu \n(Born in Oita prefecture, I'm Sashihara Rino, also known as Sasshi) \nあなたの、あなたの、あなたの指原クオリティー覚醒！　大分県出身、指原莉乃です \nAnata no, anata no, anata no Sashihara KUORITII kakusei! Oita-ken shusshin, Sashihara Rino desu \n(I'll awaken your, your, your Sashihara quality! Born in Oita prefecture, I'm Sashihara Rino)`,
            center_singles: 'AKB48 32nd Single ~ Koisuru Fortune Cookie (2013) \nAKB48 41st Single ~ Halloween Night (2015) \nAKB48 45th Single ~ Love Trip (2016) \nAKB48 49th Single ~ Sukinanda (2017) \nAKB48 55th Single ~ Jiwaru Days (2019)',

            gplus         : 'https://plus.google.com/104375100134443203420',
            instagram     : 'https://www.instagram.com/345insta/',
            nanagogo      : 'https://7gogo.jp/sashihara-rino',
            twitter       : 'https://twitter.com/345__chan',
            youtube       : 'https://www.youtube.com/channel/UCfnSEB4atVWATf6vzepHzqw',
            ameblo        : 'https://ameblo.jp/sashihara-rino/',
            vine          : 'https://vine.co/u/1049204132482789376',
            weibo         : 'https://www.weibo.com/u/6348247136?is_hot=1',
            tiktok        : '-',
            showroom      : 'https://www.showroom-live.com/Rino_SASHIHARA',
            image         : 'http://stage48.net/wiki/images/thumb/2/2d/SashiharaRinoH2018.jpg/280px-SashiharaRinoH2018.jpg',
            image2        : 'http://stage48.net/wiki/images/b/be/SashiharaRinoChristmas2018.jpg'
    },
    {
        id              : ['tomochin', 'tomomi', 'itanotomomi', 'tomomiitano'],
        name            : {
            kanji       : '板野友美',
            hiragana    : 'いたの ともみ',
            romaji      : 'Itano Tomomi'
        },
            nickname      : 'Tomochin (ともちん)',
            date_of_birth : 'July 3, 1991',
            blood_type    : 'A',
            from          : 'Kanagawa',
            office        : 'Hori Pro',
            current_teams : [ 'Former of AKB48 Team A' ],
            catch_phrase  : `お耳がピョンピョン！元気なバンビー、ともちんこと板野友美です \nOmimi ga PYONPYON! Genki na BANBII, Tomochin koto Itano Tomomi desu \n(Ears are pyon pyon (boing boing)! Energetic Bambi, Tomochin, Itano Tomomi.)`,
            center_singles: 'Sayonara Crawl',

            instagram     : 'http://instagram.com/tomo.i_0703/',
            twitter       : 'https://twitter.com/tomo_coco73',
            youtube       : 'https://www.youtube.com/channel/UCDLqvTPnN70jKZNqaDF4GwQ',
            weibo         : 'https://www.weibo.com/u/5061491055',
            tiktok        : '-',
            showroom      : '-',
            image         : 'http://stage48.net/wiki/images/thumb/4/4f/ItanoTomomi2018.jpg/280px-ItanoTomomi2018.jpg',
            image2        : 'd'
    },
    {
        id              : ['nyanchan', 'harunyan', 'kojimaharuna', 'harunakojima'],
        name            : {
            kanji       : '小嶋陽菜',
            hiragana    : '	こじま　はるな',
            romaji      : 'Kojima Haruna'
        },
            nickname      : 'Harunyan (はるにゃん), Kojiharu (こじはる)',
            date_of_birth : 'July 3, 1991  ',
            blood_type    : 'O',
            from          : 'Saitama',
            office        : 'Production Ogi',
            current_teams : [ 'Former of AKB48 Team A' ],
            catch_phrase  : `埼玉県から来ました、こじはること小嶋陽菜です。よろしくお願いします \nSaitama-ken kara kimashita, Kojiharu koto Kojima Haruna desu. Yoroshiku onegai shimasu \n(I come from Saitama prefecture, I'm Kojima Haruna also known as Kojiharu. Please be nice to me)`,
            center_singles: 'Yuuhi wo Miteiru ka? \nRomance, Irane \nSakura no Hanabiratachi 2008 \nHeart Ereki \nGreen Flash \nShoot Sign',

            instagram     : 'https://instagram.com/nyanchan22',
            twitter       : 'https://twitter.com/kojiharunyan',
            youtube       : 'https://youtube.com/channel/UCOjmYCnYlTcdoD2DKHvrV5g',
            weibo         : '-',
            tiktok        : 'https://www.tiktok.com/@harunakojima22',
            showroom      : '-',
            image         : 'http://stage48.net/wiki/images/7/75/KojimaHaruna2018.jpg'
    },
    {
        id              : ['takamina', 'takahashiminami', 'minamitakahashi'],
        name            : {
            kanji       : '高橋みなみ',
            hiragana    : 'たかはし　みなみ',
            romaji      : 'Takahashi Minami'
        },
            nickname      : 'Takamina (たかみな)',
            date_of_birth : 'April 8, 1991',
            blood_type    : 'AB',
            from          : 'Hachioji, Tokyo',
            office        : 'Production Ogi',
            current_teams : [ 'Former of AKB48 Team A' ],
            catch_phrase  : `見かけはちょっぴりヤンキー心はガラスのハートたかみなこと高橋みなみです! \nMikake wa choppiri YANKII kokoro wa GARASU no HAATO Takamina koto Takahashi Minami desu! \n(My appearance is slightly yankee, but my heart is a glass. Called Takamina, Takahashi Minami!)`,
            center_singles: 'Sakura no Hanabiratachi \nSkirt, Hirari \nSeifuku ga Jama wo Suru \nKeibetsu Shiteita Aijou \nRIVER \nSakura no Shiori \nPonytail to Shushu \nKuchibiru ni Be My Baby',

            instagram     : 'https://www.instagram.com/taka37_chan',
            twitter       : 'https://twitter.com/taka4848mina',
            youtube       : '-',
            weibo         : '-',
            tiktok        : '-',
            showroom      : '-',
            image         : 'http://stage48.net/wiki/images/4/41/TakahashiMinami2018.jpg'
    },
    {
        id              : ['acchan', 'achan', 'maedaatsuko', 'atsukomaeda'],
        name            : {
            kanji       : '前田敦子',
            hiragana    : 'まえだ あつこ',
            romaji      : 'Maeda Atsuko'
        },
            nickname      : 'Acchan (あっちゃん)',
            date_of_birth : 'July 10, 1991',
            blood_type    : 'A',
            from          : 'Ichikawa, Chiba',
            office        : 'Ohta Pro',
            current_teams : [ 'Former of AKB48 Team A' ],
            catch_phrase  : `こんばんは。前田敦子です \nKonbanwa. Maeda Atsuko desu \n(Good evening. I'm Maeda Atsuko)`,
            center_singles: `Skirt, Hirari \nAitakatta \nSeifuku ga Jama wo Suru \nBINGO! \nBoku no Taiyou \nYuuhi wo Miteiru ka? \nRomance, Irane \nSakura no Hanabiratachi 2008 \nBaby! Baby! Baby! \n10nen Zakura \nNamida Surprise! \nIiwake Maybe \nRIVER \nSakura no Shiori \nPonytail to Shushu \nHeavy Rotation \nBeginner \nSakura no Ki ni Narou \nEveryday, Kachuusha \nFlying Get \nKaze wa Fuiteiru \nGIVE ME FIVE! \nManatsu no Sounds Good!
        `,

            instagram     : 'https://www.instagram.com/atsuko_maeda_official/',
            twitter       : 'http://twitter.com/Atsuko_100',
            youtube       : '-',
            weibo         : '-',
            tiktok        : '-',
            showroom      : '-',
            image         : 'http://stage48.net/wiki/images/thumb/c/c3/MaedaAtsukoOP2018.jpg/280px-MaedaAtsukoOP2018.jpg'
    },
    {
        id              : ['miichan', 'michan', 'micchan', 'minegishiminami', 'minamiminegishi'],
        name            : {
            kanji       : '峯岸みなみ',
            hiragana    : 'みねぎし みなみ',
            romaji      : 'Minegishi Minami'
        },
            nickname      : 'Micchan (みぃちゃん)',
            date_of_birth : 'November 15, 1992  ',
            blood_type    : 'B',
            from          : 'Tokyo',
            office        : 'Production Ogi',
            current_teams : [ 'Team K' ],
            catch_phrase  : `年中無休の反抗期！　みぃちゃんこと峯岸みなみです。\nNenchuumukyuu no hankouki! Miichan koto Minegishi Minami desu. \n(Rebellious age for 7 days a week! I'm Minegishi Minami, also known as Miichan.)`,
            center_singles: '-',

            instagram     : 'https://www.instagram.com/minegishi_31chan/',
            twitter       : 'https://twitter.com/chan__31',
            youtube       : 'https://www.youtube.com/channel/UCVedolgyNM3SM6rXORDDTgg/',
            weibo         : 'https://www.weibo.com/u/6349207467?is_hot=1',
            tiktok        : 'https://www.tiktok.com/share/user/6583689225362522113',
            showroom      : 'https://www.showroom-live.com/48_Minami_Minegishi',
            image         : 'http://stage48.net/wiki/images/e/e3/MinegishiMinamiK2019.jpg'
    },
    {
        id              : ['yuuko', 'yuko', 'oshimayuko', 'oshimayuuko', 'yuukooshima', 'yukooshima'],
        name            : {
            kanji       : '大島優子',
            hiragana    : 'おおしま ゆうこ',
            romaji      : 'Oshima Yuko'
        },
            nickname      : 'Yuuko (ゆうこ), Korisu (コリス)',
            date_of_birth : 'October 17, 1988  ',
            blood_type    : 'B',
            from          : 'Mibu, Tochigi',
            office        : 'Ohta Pro',
            current_teams : [ 'Former of AKB48 Team K' ],
            catch_phrase  : `目で負かし、心で伝える変幻自在のエンターテイナー、大島優子です。よろしくお願いします \nMe de makashi, kokoro de tsutaeru hengen jizai no ENTAATEINAA, Oshima Yuko desu. Yoroshiku onegai shimasu \n(Charming you with my eyes and communicating with my heart, I'm the ever changing entertainer Oshima Yuko. Please be nice to me)`,
            center_singles: 'Heavy Rotation, Kaze wa Fuiteru, Gingham Check, UZA, Mae Shika Mukanee',

            instagram     : 'https://www.instagram.com/yuk00shima',
            twitter       : 'https://twitter.com/Oshima__Yuko',
            youtube       : '-',
            weibo         : '-',
            tiktok        : '-',
            showroom      : '-',
            image         : 'http://stage48.net/wiki/images/thumb/b/b3/OshimaYuko2018.jpg/280px-OshimaYuko2018.jpg'
    },
    {
        id              : ['sae', 'miyazawasae', 'saemiyazawa'],
        name            : {
            kanji       : '宮澤佐江',
            hiragana    : 'みやざわ さえ',
            romaji      : 'Miyazawa Sae'
        },
            nickname      : 'Sae (さえ), Sae-chan (さえちゃん)',
            date_of_birth : 'August 13, 1990  ',
            blood_type    : 'O',
            from          : 'Tokyo',
            office        : 'Hori Pro',
            current_teams : [ 'Former of SNH48 Team Kaigai, SKE48 Team S' ],
            catch_phrase  : `はい！みんなさん。。。右上げて！「はい！」左上げて！「はい！」せーの。。。てばさき！ 宮澤佐江です。よろしくお願いします。 \nHai! Mina-san migi agete! "Hai!" Hidari agete! "Hai!" se-no... tebasaki! Miyazawa Sae desu. Yoroshiku onegaishimasu. \n(Yes! Everyone, put your right hand up! "Yes!" Put your left hand up! "Yes!" 1, 2... Chicken wings! I am Miyazawa Sae. Please be nice to me.)`,
            center_singles: '-',

            instagram     : 'https://www.instagram.com/sae_3830/',
            twitter       : 'http://twitter.com/oyasuminaSAE_m',
            youtube       : '-',
            weibo         : '-',
            tiktok        : '-',
            showroom      : '-',
            image         : 'http://stage48.net/wiki/images/1/1e/MiyazawaSaeFlaveEntertainment.jpg'
    },
    {
        id              : ['yukirin', 'kashiwagiyuki', 'yukikashiwagi'],
        name            : {
            kanji       : '柏木由紀',
            hiragana    : 'かしわぎ　ゆき',
            romaji      : 'Kashiwagi Yuki'
        },
            nickname      : 'Yukirin (ゆきりん)',
            date_of_birth : 'July 15, 1991 ',
            blood_type    : 'A',
            from          : 'Kagoshima City, Kagoshima',
            office        : 'Watanabe Pro',
            current_teams : [ 'AKB48 Team B' ],
            catch_phrase  : `寝ても覚めてもゆきりんワールド、夢中にさせちゃうぞ♪　ゆきりんこと柏木由紀です。よろしくお願いします \nNete mo samete mo Yukirin WAARUDO, muchuu ni sasechau zo♪ Yukirin koto Kashiwagi Yuki desu. Yoroshiku onegai shimasu \n(Whether asleep or awake it's Yukirin world, I'll make you dazed♪ I'm Kashiwagi Yuki also known as Yukirin. Please be nice to me)`,
            center_singles: 'Green Flash',

            instagram     : 'https://www.instagram.com/yukikashiwagi_official/',
            twitter       : 'https://twitter.com/Yukiriiiin__K',
            youtube       : 'https://www.youtube.com/channel/UCubbFOa0eUCWQD8sbmQXwOQ',
            weibo         : 'https://weibo.com/u/6248701438',
            tiktok        : '-',
            showroom      : 'https://www.showroom-live.com/48_Yuki_Kashiwagi',
            image         : 'http://stage48.net/wiki/images/a/a4/KashiwagiYukiB2019.jpg'
    },
    {
        id              : ['harugon', 'haruka', 'nakagawaharuka', 'harukanakagawa'],
        name            : {
            kanji       : '仲川遥香',
            hiragana    : 'なかがわ　はるか',
            romaji      : 'Nakagawa Haruka'
        },
            nickname      : 'Haruka (はるか）',
            date_of_birth : 'February 10, 1992  ',
            blood_type    : 'O',
            from          : 'Tokyo',
            office        : 'dentsuXentertainment',
            current_teams : [ 'Former of JKT48 Team Kaigai (Team J)' ],
            catch_phrase  : `Kalian cinta aku? (Ya), aku cinta kalian semua. Jauh jauh datang dari Jepang untuk menghibur kalian semua, aku Nakagawa Haruka \n(Do you love me? (Audience: Yes), I love you all. Came far away from Japan to entertain you, I'm Nakagawa Haruka)`,
            center_singles: 'Koi Suru Fortune Cookie',

            instagram     : 'https://instagram.com/haruuuu_chan',
            twitter       : 'https://twitter.com/Haruka_NKGW10',
            youtube       : '-',
            weibo         : '-',
            tiktok        : '-',
            showroom        : '-',
            image         : 'http://stage48.net/wiki/images/thumb/4/4f/HarukaAfterGraduation2017.jpg/140px-HarukaAfterGraduation2017.jpg'
    },
    {
        id              : ['mayuyu', 'watanabemayu', 'mayuwatanabe'],
        name            : {
            kanji       : '渡辺麻友',
            hiragana    : 'わたなべ　まゆ',
            romaji      : 'Watanabe Mayu'
        },
            nickname      : 'Mayuyu (まゆゆ)',
            date_of_birth : 'March 26, 1994  ',
            blood_type    : 'AB',
            from          : 'Saitama City, Saitama',
            office        : '-',
            current_teams : [ 'Former of AKB48 Team B' ],
            catch_phrase  : `み〜んなの目線を、いただきまゆゆ~まゆゆこと渡辺麻友です \nMinna no mesen wo, itadaki Mayuyu~ Mayuyu koto Watanabe Mayu desu \n(Receiving everyone’s sights, Mayuyu~ Mayuyu, also known as Watanabe Mayu)`,
            center_singles: 'So long!, Labrador Retriever, Kokoro no Placard, Kibouteki Refrain, 11gatsu no Anklet',

            instagram     : '-',
            twitter       : '-',
            youtube       : '-',
            weibo         : '-',
            tiktok        : '-',
            showroom      : '-',
            image         : 'http://stage48.net/wiki/images/thumb/1/16/WatanabeMayu2018.png/280px-WatanabeMayu2018.png'
    },
    {
        id              : ['kitarie', 'kitahararie', 'riekitahara'],
        name            : {
            kanji       : '北原里英',
            hiragana    : 'きたはら りえ',
            romaji      : 'Kitahara Rie'
        },
            nickname      : 'Kitarie (きたりえ), Rie (りえ)',
            date_of_birth : 'June 24, 1991  ',
            blood_type    : 'A',
            from          : 'Ichinomiya City, Aichi',
            office        : 'Ohta Pro',
            current_teams : [ 'Former of NGT48 Team NIII' ],
            catch_phrase  : `愛知県から来ました、北原里英、29歳です。よろしくお願いします \nAichi-ken kara kimashita, Kitahara Rie, 29sai desu. Yoroshiku onegai shimasu \n(I come from Aichi prefecture, I'm Kitahara Rie, 29 years old. Please be nice to me)`,
            center_singles: '-',

            instagram     : 'https://www.instagram.com/rie_kitahara_3/',
            twitter       : 'https://twitter.com/Rie_Kitahara3',
            youtube       : 'https://www.youtube.com/channel/UCFyJZ_ogMdPJu88AukWt62w',
            weibo         : 'http://www.weibo.com/u/6352282338?is_hot=1',
            tiktok        : '-',
            showroom      : '-',
            image         : 'http://stage48.net/wiki/images/thumb/d/de/KitaharaRieNIII2018.jpg/280px-KitaharaRieNIII2018.jpg'
    },
    {
        id              : ['yuihan','yokoyamayui', 'yuiyokoyama'],
        name            : {
            kanji       : '横山由依',
            hiragana    : 'よこやま ゆい',
            romaji      : 'Yokoyama Yui'
        },
            nickname      : 'Yui (ゆい), Yuihan (ゆいはん)',
            date_of_birth : 'December 8, 1992  ',
            blood_type    : 'B',
            from          : 'Kizugawa City, Kyoto',
            office        : 'Ohta Pro',
            current_teams : [ 'AKB48 Team A' ],
            catch_phrase  : `京都から来ました、金閣寺よりも輝きたい、横山由依、28歳です。よろしくお願いします \nKyoto kara kimashita, Kinkakuji yori mo kagayakitai, Yokoyama Yui, 28 desu. Yoroshiku onegai shimasu \n(I came from Kyoto, I want to shine more than the Golden Pavillion, I'm Yokoyama Yui, 28 years old. Please be nice to me)`,
            center_singles: '-',

            instagram     : 'https://www.instagram.com/yokoyamayui_1208/',
            twitter       : 'https://twitter.com/Yui_yoko1208',
            youtube       : 'https://www.youtube.com/channel/UCqZkbT3ZfO93GetgFqT816A',
            weibo         : 'https://www.weibo.com/u/5536924142?is_hot=1',
            tiktok        : 'https://www.tiktok.com/share/user/78951104716',
            showroom      : 'https://www.showroom-live.com/48_Yui_Yokoyama_A',
            image         : 'http://stage48.net/wiki/images/thumb/3/38/YokoyamaYuiA2019.jpg/280px-YokoyamaYuiA2019.jpg'
    },
    {
        id              : ['paruru', 'shimazakiharuka', 'harukashimazaki'],
        name            : {
            kanji       : '島崎遥香',
            hiragana    : '-',
            romaji      : 'Shimazaki Haruka'
        },
            nickname      : 'Paruru (ぱるる)',
            date_of_birth : 'March 30, 1994 ',
            blood_type    : 'A',
            from          : 'Tsurugashima, Saitama',
            office        : 'Big Apple',
            current_teams : [ 'Former of AKB48 Team A' ],
            catch_phrase  : `チームA、島崎遥香です \nCHIIMU A, Shimazaki Haruka desu \n(I'm Team A's Shimazaki Haruka)`,
            center_singles: 'Sayonara Crawl, Bokutachi wa Tatakawanai, High Tension',

            instagram     : 'https://www.instagram.com/paruuuuuru/',
            twitter       : 'https://twitter.com/paruruchan0330',
            youtube       : 'https://www.youtube.com/channel/UCaW_iei_YZRuUogGFOXofMw',
            weibo         : 'https://weibo.com/u/7448414747',
            tiktok        : 'https://www.tiktok.com/@paruru866',
            showroom      : 'https://www.showroom-live.com/paruru_free',
            image         : 'http://stage48.net/wiki/images/thumb/3/37/ShimazakiHaruka2016.jpg/280px-ShimazakiHaruka2016.jpg'
    },
    {
        id              : ['juri', 'takahashijuri', 'juritakahashi'],
        name            : {
            kanji       : '	高橋朱里',
            hiragana    : 'たかはし じゅり',
            romaji      : 'Takahashi Juri'
        },
            nickname      : 'Juri (じゅり)',
            date_of_birth : 'October 3, 1997  ',
            blood_type    : 'A',
            from          : 'Kashima, Ibaraki',
            office        : 'Woolim Entertainment',
            current_teams : [ 'Former of AKB48 Team B' ],
            catch_phrase  : `おはし, わたがし, やつはし, たかはし, みんながいちばんすきなのわ? たかはし! チームAの23歳、高橋朱里です \nOhashi, watagashi, yatsuhashi, Takahashi, minna ga ichiban suki na no wa? Takahashi! CHIIMU A no 23sai, Takahashi Juri desu \n(Chopsticks, cotton candy, yatsuhashi, Takahashi, which one is everyone's favorite? Takahashi! 23 years old from Team A, Takahashi Juri)`,
            center_singles: '-',

            instagram     : 'https://instagram.com/official_rcpc_',
            twitter       : 'http://twitter.com/RCPC_members',
            youtube       : '-',
            weibo         : '-',
            tiktok        : '-',
            showroom      : '-',
            image         : 'http://stage48.net/wiki/images/thumb/3/3a/TakahashiJuri2018.jpg/280px-TakahashiJuri2018.jpg'
    },
    {
        id              : ['yuiri', 'murayamayuiri', 'yuirimurayama'],
        name            : {
            kanji       : '村山彩希',
            hiragana    : 'むらやま ゆいり',
            romaji      : 'Murayama Yuiri'
        },
            nickname      : 'Yuirii (ゆいりー)',
            date_of_birth : 'June 15, 1997  ',
            blood_type    : 'O',
            from          : 'Kanagawa',
            office        : 'DH',
            current_teams : [ 'AKB48 Team 4' ],
            catch_phrase  : `真っ赤な..、ゆいりんご！ \nMakkana.., Yuiringo! \n(Bright red, Yuiringo!)`,
            center_singles: '-',

            instagram     : 'https://instagram.com/akb48',
            twitter       : 'https://twitter.com/yuirii_murayama',
            youtube       : 'https://www.youtube.com/channel/UC0X6o-aELH_Rt3ZXLoKxe-w',
            weibo         : '-',
            tiktok        : '-',
            showroom      : 'https://www.showroom-live.com/48_Yuiri_Murayama',
            image         : 'http://stage48.net/wiki/images/thumb/3/3c/MurayamaYuiri42019.jpg/280px-MurayamaYuiri42019.jpg'
    },
    {
        id              : ['nachan', 'nacchan', 'okadanana', 'nanaokada'],
        name            : {
            kanji       : '岡田奈々',
            hiragana    : 'おかだ なな',
            romaji      : 'Okada Nana'
        },
            nickname      : 'Naachan (なぁちゃん)',
            date_of_birth : 'November 7, 1997  ',
            blood_type    : 'A',
            from          : 'Neyagawa, Osaka',
            office        : 'DH',
            current_teams : [ 'AKB48 Team 4' ],
            catch_phrase  : `-`,
            center_singles: 'Jabaja',

            instagram     : 'https://www.instagram.com/okada7_akb48_stu48/',
            twitter       : 'https://twitter.com/okadanana_1107',
            youtube       : 'https://www.youtube.com/channel/UC0X6o-aELH_Rt3ZXLoKxe-w',
            weibo         : '-',
            tiktok        : 'https://www.tiktok.com/share/user/6542342467467182081',
            showroom      : 'https://www.showroom-live.com/48_Nana_Okada',
            image         : 'http://stage48.net/wiki/images/thumb/9/9b/OkadaNana42019.jpg/280px-OkadaNana42019.jpg'
    },
    {
        id              : ['mion', 'mukaichimion', 'mionmukaichi'],
        name            : {
            kanji       : '向井地美音',
            hiragana    : 'むかいち みおん',
            romaji      : 'Mukaichi Mion'
        },
            nickname      : 'Miion (みーおん)',
            date_of_birth : 'January 29, 1998  ',
            blood_type    : 'O',
            from          : 'Saitama',
            office        : 'Mama & Son Inc.',
            current_teams : [ 'AKB48 Team A' ],
            catch_phrase  : `-`,
            center_singles: 'Tsubasa wa Iranai',

            instagram     : 'https://www.instagram.com/___mion.m/',
            twitter       : 'https://twitter.com/mionnn_48',
            youtube       : 'https://www.youtube.com/channel/UC0X6o-aELH_Rt3ZXLoKxe-w',
            weibo         : '-',
            tiktok        : '-',
            showroom      : 'https://www.showroom-live.com/48_Mion_Mukaichi',
            image         : 'http://stage48.net/wiki/images/thumb/3/3f/MukaichiMionA2019.jpg/280px-MukaichiMionA2019.jpg'
    },
    {
        id              : ['zukky', 'zukki', 'yamauchimizuki', 'mizukiyamauchi'],
        name            : {
            kanji       : '山内瑞葵',
            hiragana    : 'やまうち みずき',
            romaji      : 'Yamauchi Mizuki'
        },
            nickname      : 'Zukkii (ずっきー)',
            date_of_birth : 'September 20, 2001 ',
            blood_type    : 'O',
            from          : 'Tokyo',
            office        : 'DH',
            current_teams : [ 'AKB48 Team 4' ],
            catch_phrase  : `-`,
            center_singles: 'Shitsuren, Arigatou',

            instagram     : 'https://www.instagram.com/zukky_48/',
            twitter       : 'https://twitter.com/MizukiYamauchi',
            youtube       : '-',
            weibo         : '-',
            tiktok        : '-',
            showroom      : 'https://www.showroom-live.com/48_Mizuki_Yamauchi',
            image         : 'http://stage48.net/wiki/images/thumb/3/31/YamauchiMizuki42019.jpg/280px-YamauchiMizuki42019.jpg'
    },
    {
        id              : ['yuiyui', 'oguriyui', 'yuioguri'],
        name            : {
            kanji       : '小栗有以',
            hiragana    : 'おぐり ゆい',
            romaji      : 'Oguri Yui'
        },
            nickname      : 'Yuiyui (ゆいゆい)',
            date_of_birth : 'December 26, 2001',
            blood_type    : '-',
            from          : 'Tokyo',
            office        : 'DH',
            current_teams : [ 'AKB48 Team 8' ],
            catch_phrase  : `みーんなのハートを（取っちゃう、取っちゃう） 東京から来ました"ゆいゆい"こと小栗有以です \nMi~nna no HAATO wo (Tocchau, tocchau) Tokyo kara kimashita "Yuiyui" koto Oguri Yui desu \n(Everyone's hearts (I'll take them, I'll take them) I came from Tokyo, Oguri Yui known as "Yuiyui")`,
            center_singles: 'Teacher Teacher',

            instagram     : 'https://www.instagram.com/yuioguri_1226',
            twitter       : 'https://twitter.com/yuiyui_maromaro',
            youtube       : '-',
            weibo         : '-',
            tiktok        : 'https://www.tiktok.com/share/user/6599613065690415105',
            showroom      : 'https://www.showroom-live.com/48_YUI_OGURI',
            image         : 'http://stage48.net/wiki/images/thumb/7/71/OguriYui8June2019.png/280px-OguriYui8June2019.png'
    },
    {
        id              : ['sakura', 'kkura', 'miyawakisakura', 'sakuramiyawaki'],
        name            : {
            kanji       : '宮脇咲良',
            hiragana    : 'みやわき　さくら',
            romaji      : 'Miyawaki Sakura'
        },
            nickname      : 'Sakura (さくら)',
            date_of_birth : 'March 19, 1998  ',
            blood_type    : 'A',
            from          : 'Kagoshima City, Kagoshima',
            office        : 'Mercury, Off the Record',
            current_teams : [ 'HKT48 Team KIV, IZ*ONE' ],
            catch_phrase  : `あなたも、あなたも、みんなの心に（さくら咲けー！）　鹿児島県出身、宮脇咲良22歳です。 \nAnata mo, anata mo, minna no kokoro ni (Sakura Sake!) Kagoshima-ken shusshin, Miyawaki Sakura 22-sai desu. \n(You too, you too, in everyone's heart (Sakura bloom!) I'm Miyawaki Sakura, I'm 22 years old, from Kagoshima prefecture.)`,
            center_singles: 'Kibouteki Refrain, Kimi wa Melody, Negaigoto no Mochigusare, NO WAY MAN',

            instagram     : 'https://www.instagram.com/39saku_chan/',
            twitter       : 'https://twitter.com/39saku_chan/',
            youtube       : 'https://www.youtube.com/channel/UC9Z0cgZPBa2szfJcrJdfYvA',
            weibo         : 'http://www.weibo.com/u/6352047195?is_hot=1',
            tiktok        : 'https://www.tiktok.com/share/user/6542341154200567810',
            showroom      : 'http://www.showroom-live.com/48_SAKURA_MIYAWAKI',
            image         : 'http://stage48.net/wiki/images/thumb/b/bd/MiyawakiSakuraBuenosAires2019.jpeg/280px-MiyawakiSakuraBuenosAires2019.jpeg'
    }
]


exports.AKBG = AKBG;